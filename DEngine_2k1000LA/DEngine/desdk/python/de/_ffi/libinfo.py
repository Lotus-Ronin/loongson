...
def find_lib_path(name=None, search_path=None, optional=False):
    ...
    dll_path.append(os.path.join(source_dir, "build", "Debug"))
    dll_path.append(os.path.join(source_dir, "build", "Release"))
    dll_path.append(os.path.join(source_dir, "build", "lib", "Debug"))
    dll_path.append(os.path.join(source_dir, "build", "lib", "Release"))
    dll_path.append(os.path.join(source_dir, "build", "host", "lib", "Debug"))
    dll_path.append(os.path.join(source_dir, "build", "host", "lib", "Release"))
    # Default build_runtime.sh directory
    dll_path.append(os.path.join(source_dir, "output", "x86_linux_host", "lib"))
    ...
    if name is not None:
        ...
        if sys.platform.startswith('win32'):
            runtime_dll_path = [os.path.join(p, 'desdk.dll') for p in dll_path]
        elif sys.platform.startswith('darwin'):
            runtime_dll_path = [os.path.join(p, 'libdesdk.dylib') for p in dll_path]
        else:
            runtime_dll_path = [os.path.join(p, 'libdesdk.so') for p in dll_path]
