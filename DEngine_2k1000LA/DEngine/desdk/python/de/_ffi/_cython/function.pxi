...
from ..runtime_ctypes import DEType, DEContext, DEByteArray
...
cdef inline bytearray make_ret_bytes(void* chandle):
    ...
    arr = ctypes.cast(handle, ctypes.POINTER(DEByteArray))[0]
    size = arr.size
    res = bytearray(size)
...
cdef inline object make_ret(TVMValue value, int tcode):
    ...
    elif tcode == kDEContext:
        return DEContext(value.v_ctx.device_type, value.v_ctx.device_id)
