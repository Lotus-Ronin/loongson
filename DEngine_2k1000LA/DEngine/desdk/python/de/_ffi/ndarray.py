...
from .runtime_ctypes import DEType, DEContext, DEArray, DEArrayHandle, DENDArrayExtInfo, DENDArrayExtInfoHandle, Rect
from .runtime_ctypes import TypeCode, de_shape_index_t
...
def _ext_info(format=0, shape_indicator=0, roi_rect = Rect(0,0,0,0)):
    """Construct a addition info obj.
    ...
    Returns
    -------
    info: DENDArrayExtInfo

    """
    return DENDArrayExtInfo(format, shape_indicator, roi_rect)
...
def context(dev_type, dev_id=0):
    ...
    if isinstance(dev_type, string_types):
        ...
        if dev_type not in DEContext.STR2MASK:
            raise ValueError("Unknown device type %s" % dev_type)
        dev_type = DEContext.STR2MASK[dev_type]
    return DEContext(dev_type, dev_id)


def numpyasarray(np_data):
    """Return a DEArray representation of a numpy array.
    """
    data = np_data
    assert data.flags['C_CONTIGUOUS']
    arr = DEArray()
    shape = c_array(de_shape_index_t, data.shape)
    arr.data = data.ctypes.data_as(ctypes.c_void_p)
    arr.shape = shape
    arr.strides = None
    arr.dtype = DEType(np.dtype(data.dtype).name)
    arr.ndim = data.ndim
    # CPU device
    arr.ctx = context(1, 0)
    return arr, shape
...
def empty(shape, dtype="float32", ctx=context(1, 0)):
    ...
    """
    shape = c_array(de_shape_index_t, shape)
    ndim = ctypes.c_int(len(shape))
    handle = DEArrayHandle()
    dtype = DEType(dtype)
    check_call(_LIB.DEArrayAlloc(
        shape, ndim,
...
def empty_dp(shape, dtype="float32", ctx=context(1, 0), info=_ext_info(0, 0, roi_rect=(0, 0, 0, 0))):
    """Create an empty array given shape and device
    ...
    handle = DEArrayHandle()
    dtype = DEType(dtype)
    check_call(_LIB.DEArrayAllocDp(
        shape, ndim,
        ctypes.c_int(dtype.type_code),
        ctypes.c_int(dtype.bits),
        ctypes.c_int(dtype.lanes),
...
class NDArrayBase(_NDArrayBase):
    ...
    @property
    def ext_info(self):
        """ext info of this array"""   
        info = DENDArrayExtInfoHandle()
        check_call(_LIB.DEArrayExtInfo(self.handle, ctypes.byref(info)))
        return info
    ...
    def copyfrom(self, source_array):
        ...
        check_call(_LIB.DEArrayCopyFromBytes(self.handle, data, nbytes))
        return self
    ...
    def asnumpy(self):
        ...
        check_call(_LIB.DEArrayCopyToBytes(self.handle, data, nbytes))
        return np_arr
    ...
    def copyto(self, target):
        ...
        """
        if isinstance(target, DEContext):
            target = empty(self.shape, self.dtype, target)
        if isinstance(target, NDArrayBase):
            check_call(_LIB.DEArrayCopyFromTo(
...
def free_extension_handle(handle, type_code):
    ...
    type_code : int
         The tyoe code
    """
    check_call(_LIB.DEExtTypeFree(handle, ctypes.c_int(type_code)))
