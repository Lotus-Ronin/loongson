...
def _load_lib():
    ...
    desdk_lib = lib_path[0]
    lib_dir_name = os.path.dirname(desdk_lib)

    if sys.platform.startswith('win32'):
        lib_file_list = glob.glob(lib_dir_name + '/*.dll')
    elif sys.platform.startswith('darwin'):
        lib_file_list = glob.glob(lib_dir_name + '/*..dylib')
    else:
        lib_file_list = glob.glob(lib_dir_name + '/*.so')
    
    #Do not load libgstlive555src at startup time.
    lib_file_list.remove(lib_dir_name + '/libdesdkmm.so')
    lib_file_list.remove(lib_dir_name + '/libgstlive555src.so')
    lib_num = len(lib_file_list)
    ...
    while(i < lib_num):
        for lib_file in lib_file_list:
            try:
                RTLD_LAZY = 0x0001
                LAZYLOAD = RTLD_LAZY | ctypes.RTLD_GLOBAL
                lib_temp = ctypes.CDLL(lib_file, LAZYLOAD)
                if(lib_file == desdk_lib):
                    lib = lib_temp
                lib_file_list.remove(lib_file)
                print("load " + lib_file + " success")
            except:
                pass
        i = i + 1

    for lib_file in lib_file_list:
        ctypes.CDLL(lib_file, ctypes.RTLD_GLOBAL)

    lib = ctypes.CDLL(desdk_lib, ctypes.RTLD_GLOBAL)
    # DMatrix functions
    lib.DEGetLastError.restype = ctypes.c_char_p
...
# Whether we are desdk only
_DESDK_ONLY = "desdk" in _LIB_NAME
...
class DEError(RuntimeError):
    """Default error thrown by TVM functions.

    DEError will be raised if you do not give any error type specification,
    """
