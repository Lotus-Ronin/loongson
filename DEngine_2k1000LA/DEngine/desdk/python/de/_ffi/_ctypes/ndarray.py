...
from ..runtime_ctypes import DEArrayHandle, DENDArrayContainerHandle
...
def _from_dlpack(dltensor):
    ...
    if ctypes.pythonapi.PyCapsule_IsValid(dltensor, _c_str_dltensor):
        ...
        check_call(_LIB.DEArrayFromDLPack(ptr, ctypes.byref(handle)))
...
def _dlpack_deleter(pycapsule):
    ...
    if ctypes.pythonapi.PyCapsule_IsValid(pycapsule, _c_str_dltensor):
        ...
        _LIB.DEDLManagedTensorCallDeleter(ptr)
        ctypes.pythonapi.PyCapsule_SetDestructor(dltensor, TVMPyCapsuleDestructor(0))
...
class NDArrayBase(object):
    ...
    def __del__(self):
        if not self.is_view and _LIB:
            check_call(_LIB.DEArrayFree(self.handle))
    ...
    def to_dlpack(self):
        ...
        check_call(_LIB.DEArrayToDLPack(self.handle, ctypes.byref(handle)))
...
def _make_array(handle, is_view, is_container):
    global _TVM_ND_CLS
    handle = ctypes.cast(handle, DEArrayHandle)
    fcreate = _CLASS_NDARRAY
    if is_container and _TVM_ND_CLS:
        array_type_info = ctypes.cast(handle, DENDArrayContainerHandle).array_type_info.value
