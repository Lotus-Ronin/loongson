...
from ..runtime_ctypes import DEType, DEByteArray, DEContext
...
def convert_to_tvm_func(pyfunc):
    ...
    def cfun(args, type_codes, num_args, ret, _):
        ...
        try:
            ...
            msg = py2cerror(msg)
            _LIB.DEAPISetLastError(c_str(msg))
            return -1
        ...
        if rv is not None:
            ...
            if _LIB.DECFuncSetReturn(ret, values, tcodes, ctypes.c_int(1)) != 0:
...
def _make_tvm_args(args, temp_args):
    ...
    for i, arg in enumerate(args):
        ...
        elif isinstance(arg, DEType):
            values[i].v_str = c_str(str(arg))
            type_codes[i] = TypeCode.STR
        elif isinstance(arg, DEContext):
...
class FunctionBase(object):
    ...
    def __del__(self):
        if not self.is_global and _LIB is not None:
            if _LIB.DEFuncFree(self.handle) != 0:
                raise get_last_ffi_error()
