...
class ModuleBase(object):
    ...
    def __del__(self):
        check_call(_LIB.DEModFree(self.handle))
    ...
    def get_function(self, name, query_imports=False):
        ...
        check_call(_LIB.DEModGetFunction(
    ...
    def import_module(self, module):
        ...
        module : Module
            The other module.
        """
        check_call(_LIB.DEModImport(self.handle, module.handle))
...
def register_func(func_name, f=None, override=False):
    ...
    def register(myf):
        ...
        check_call(_LIB.DEFuncRegisterGlobal(
...
def get_global_func(name, allow_missing=False):
    ...
    check_call(_LIB.DEFuncGetGlobal(c_str(name), ctypes.byref(handle)))
    if handle.value:
...
def get_remote_func(dev_id, name, query_imports=False):
    """Get function from device id
    ...
    """
    ret_handle = FunctionHandle()
    check_call(_LIB.DEDeviceGetFunction(
        dev_id, c_str(name),
        ctypes.c_int(query_imports),
        ctypes.byref(ret_handle)))
    if not ret_handle.value:
        raise AttributeError(
            "Device has no function '%s'" %  name)
    return Function(ret_handle, False)


def get_global_func_handle(name, allow_missing=False):
    """Get a global function by name
...
def list_global_func_names():
    ...
    check_call(_LIB.DEFuncListGlobalNames(ctypes.byref(size),
                                           ctypes.byref(plist)))
    fnames = []
...
def _init_api(namespace, target_module_name=None):
    ...
    if namespace.startswith("de."):
        _init_api_prefix(target_module_name, namespace[3:])
    else:
    ...
    for name in list_global_func_names():
        if prefix == "api":
            ...
            if name.startswith("_"):
                target_module = sys.modules["de._api_internal"]
            else:
        ...
        ff = _get_api(f)
        ff.__name__ = fname
        ff.__doc__ = ("DE PackedFunc %s. " % fname)
