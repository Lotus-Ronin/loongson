...
from .._ffi.runtime_ctypes import DEContext
...
def create(graph_json_str, libmod, ctx):
    ...
    ctx : DEContext or list of DEContext
        The context to deploy the module. It can be local or remote when there
        is only one DEContext. Otherwise, the first context in the list will
...
def get_device_ctx(libmod, ctx):
    ...
    Returns
    -------
    ctx : list of DEContext
    num_rpc_ctx : Number of rpc contexts
    device_type_id : List of device type and device id
    """

    if isinstance(ctx, DEContext):
        ctx = [ctx]
    elif not isinstance(ctx, (list, tuple)):
        raise ValueError("ctx has to be the type of DEContext or a list of "
                         "TVMCDEContext")
    for cur_ctx in ctx:
        if not isinstance(cur_ctx, DEContext):
            raise ValueError("ctx has to be the type of DEContext or a list "
                             "of DEContext")
