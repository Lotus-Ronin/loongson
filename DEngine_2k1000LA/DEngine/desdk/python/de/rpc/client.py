...
from .._ffi.base import DEError
from .._ffi.base import _LIB
...
class RPCSession(object):
    ...
    def dp1000_time_evaluator(self, func_name, number=10, repeat=1, min_repeat_ms=0):
        ...
        """
        ctx = self.cpu(0)
        return self._sess.time_evaluator(func_name, ctx, number, repeat, min_repeat_ms)
    ...
    def upload(self, data, target=None):
        ...
        if "upload" not in self._remote_funcs:
            self._remote_funcs["upload"] = self.get_function(
                "de.rpc.server.upload")
        ret = self._remote_funcs["upload"](target, blob)

        #prevent excessive memory usaged due to exessive file size
        gc.collect()
        return ret
...
def connect(dev_id, url, port, key="", session_timeout=0):
    ...
    _LIB.DEInitDeviceConfig(dev_id, bytes(url, "utf-8"), port)
    
    try:
        if session_timeout:
            key += " -timeout=%s" % str(session_timeout)
        sess  = base._Connect(dev_id, url, port, key)
    except NameError:
        raise RuntimeError("Please compile with USE_RPC=1")

    return RPCSession(sess)

def get_session(dev_id):
    """Get remote session from device id
    ...
    """
    fun = function.get_global_func('rpc.module.from.devid')
    sess = fun(dev_id)
    return RPCSession(dev_id, sess)
