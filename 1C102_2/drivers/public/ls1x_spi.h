...
void Spi_FreqDiv(uint8_t spr);  // SPI 分频系数配置 分别配置到控制寄存器、外部寄存器、参数控制寄存器
void Spi_Init(uint8_t spr);     // SPI 初始化
uint16_t Spiflash_Rdid(void);
void Spi_Write_Read_1to4(uint8_t* buf, int n);//spi 读、写 1~4字节
void Spi_Write_Read_N(uint8_t* buf, int n);//spi 读 写 N字节
void Spi_Send(uint8_t* buf, int n);//SPI 发送数据
